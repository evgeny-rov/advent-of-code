...
const simulate = (map: string[][]) => {
  ...
  while (true) {
    ...
    if (y === sandSource[1] && !unoccupiedTile) return ++settled;
...
const floor = Math.max(...paths.flat().map(([, y]) => y)) + 2;
const map = createMap(paths);
map[floor].fill('#');
const result = simulate(map);

await Deno.writeTextFile('output-part2.txt', map.map((row) => row.join('')).join('\n'));
console.log(result);
